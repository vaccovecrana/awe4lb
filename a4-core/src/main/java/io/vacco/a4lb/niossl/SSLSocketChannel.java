...
public class SSLSocketChannel extends SocketChannel {
  ...
  public SSLSocketChannel(SocketChannel socketChannel, final SSLEngine sslEngine, ExecutorService executorService) {
    ...
    this.sslEngineBuffer = new SSLEngineBuffer(socketChannel, sslEngine, executorService);
  }
  ...
  synchronized public int read(ByteBuffer applicationBuffer) throws IOException, IllegalArgumentException {
    if (log.isTraceEnabled()) {
      log.trace("read: " + applicationBuffer.position() + " " + applicationBuffer.limit());
    }
    int intiPosition = applicationBuffer.position();
    int readFromChannel = sslEngineBuffer.unwrap(applicationBuffer);
    if (log.isTraceEnabled()) {
      log.trace("read: from channel: " + readFromChannel);
    }
    if (readFromChannel < 0) {
      if (log.isTraceEnabled()) {
        log.trace("read: channel closed.");
      }
      return readFromChannel;
    } else {
      int totalRead = applicationBuffer.position() - intiPosition;
      if (log.isTraceEnabled()) {
        log.trace("read: total read: " + totalRead);
      }
      return totalRead;
    }
  }
  ...
  synchronized public int write(ByteBuffer applicationBuffer) throws IOException, IllegalArgumentException {
    ...
    if (writtenToChannel < 0) {
      if (log.isTraceEnabled()) {
        log.trace("write: channel closed");
      }
      return writtenToChannel;
    } else {
      int totalWritten = applicationBuffer.position() - intialPosition;
      if (log.isTraceEnabled()) {
        log.trace("write: total written: " + totalWritten + " amount available in network outbound: " + applicationBuffer.remaining());
      }
      return totalWritten;
    }
  }
   ...
   * @throws java.nio.channels.ClosedByInterruptException If another thread interrupts the current thread while the read operation is in progress, thereby closing the channel and setting the current thread's interrupt status
   * @throws IOException                                  If some other I/O error occurs
   * @throws IllegalArgumentException                     If one of the given applicationBuffers capacity ({@link ByteBuffer#capacity()} is less than the application buffer size of the {@link SSLEngine} session application buffer size ({@link SSLSession#getApplicationBufferSize()} this channel was constructed was.
   */
  @Override
